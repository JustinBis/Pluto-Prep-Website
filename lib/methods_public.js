...
Meteor.methods({
	...
	checkQuizAnswer: function(quiz_id, question_id, answer, time) {
		...
		UserInteraction.questionAnswered(this.userId, question_id, answer, correct, time);
		...
		if(!this.isSimulation && Meteor.call('hasPlayerFinishedQuiz', quiz_id))
		{
			...
			set = {};
			// Note: time_finish also acts as a marker to mark the quiz as done in the DB
			set['time_finish'] = (new Date()).getTime();
			// Calculate the total score
			set['total_score'] = Meteor.call()
