...
var NonEmptyStrings = Match.Where(function (x) {
	for (var key in x) {
		// No need to check if non-empty string for premium field.
		if (key !== "premium") {
...
var ValidTopic = Match.Where(function (x) {
	...
	"Lymphatic System", "Immune System", "Digestive System","Excretory System","Reproductive System","Muscle System",
...
Meteor.methods({
	...
	// Now takes in when the question was started as a parameter.
	checkQuizAnswer: function(quiz_id, question_id, answer, quesStartTime) {
		...
		set[field] = points;
		// Set boolean of if this question was answered correctly or not, used in review
		// page to sort questions by inc/correct.
		field = "questions.$."+player+"_cor";
		set[field] = correct;
