...
Router.configure({
	...
	subscriptions: function() {
		return [
			...
			Meteor.subscribe("myQuestionTracker"),
			Meteor.subscribe("myQuizzes"),
		];
	},
});
...
Router.route('/', {
	...
	// When page unloads, save view duration of this page.
	unload: function() {
		Localytics.llViewDuration("Home Page");
	},
});

// Route for review page by subject
Router.route('/dashboard/review/:subject', {
	name: 'app-dashboard-review',
	template: 'appDashboardReview',
	data: function() {
		if (this.params.subject === "all") {
			console.log("find all");
			templateData = { quizzes: Quizzes.find({})};
		}
		else {
			console.log("nuh");
			templateData = { quizzes: Quizzes.find({subject: this.params.subject}) };
		}
		return templateData;
	},
	action: function() {
...
// Router for general review page (display all quizzes)
Router.route('/dashboard/review/', function() {
	console.log("render new stuff");
	this.render('app-dashboard-review', {subject: "all"});
});
...
Router.route('/newquiz/:subject', {
	...
	// When page unloads, save view duration of this page.
	unload: function() {
		Localytics.llViewDuration("Find Partner Page");
	},
});
...
Router.route('/newquiz', {
	...
	// When page unloads, save view duration of this page.
	unload: function() {
		Localytics.llViewDuration("Select Subject Page");
	},
});
...
Router.route('/quiz/:_id/results', {
	...
	// When page unloads, save view duration of this page.
	unload: function() {
		Localytics.llViewDuration("Results Page");
	},
});
